...
- When a Playwright test fails, do NOT fix it silently. First ask whether to heal
  inline (in the main conversation) or delegate to the `playwright-test-healer`
  subagent, then proceed with the chosen approach.
- Regardless of approach, follow the healer workflow: reproduce → read the error →
  inspect the live app → identify root cause → apply the minimum-viable fix →
  re-run to confirm. Never weaken assertions, add `waitForTimeout`, or skip the test.
